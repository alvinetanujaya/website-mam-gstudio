...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const [isSeedingOrders, setIsSeedingOrders] = useState<boolean>(false);
  ...
  const sqlSchema = `-- =========================================================
-- SKRIP SQL TERBARU UNTUK DATABASE SUPABASE MAM CULINARY HERITAGE
-- (Termasuk RLS Policy, Table Products, Orders, & Order Items Lengkap)
-- Salin seluruh skrip ini lalu paste di SQL Editor Supabase
-- =========================================================

-- 1. BERSIHKAN TABEL SEBELUMNYA (RESET TOTAL & AMAN)
DROP TABLE IF EXISTS order_items CASCADE;
DROP TABLE IF EXISTS orders CASCADE;
DROP TABLE IF EXISTS products CASCADE;
DROP TABLE IF EXISTS admin_settings CASCADE;

-- 2. DUKUNGAN EKSTENSI UUID
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 3. TABEL PRODUK / KATALOG MENU & STOK
...
-- 5. TABEL RINCIAN PESANAN (ORDER_ITEMS)
-- product_name disimpan langsung agar detail pesanan tidak hilang jika menu dihapus
...
-- 6. TABEL SETTINGS ADMIN (PIN SECURITY)
...
INSERT INTO admin_settings (key, value) VALUES ('admin_pin', '1234')
ON CONFLICT (key) DO UPDATE SET value = '1234';

-- 7. SEED DATA: 14 MENU UTAMA & FROZEN FOOD
...
-- 8. SEED DATA CONTOH TRANSAKSI & RINCIAN ORDER ITEMS LENGKAP
INSERT INTO orders (id, customer_name, customer_phone, total_amount, status, created_at) VALUES
('ORDER-2026-001', 'Budi Santoso', '081234567890', 110000, 'settlement', NOW() - INTERVAL '2 hours'),
('ORDER-2026-002', 'Siti Rahma', '085712345678', 215000, 'settlement', NOW() - INTERVAL '1 day'),
('ORDER-2026-003', 'Ahmad Hidayat', '082198765432', 80000, 'pending_wa', NOW() - INTERVAL '30 minutes');

INSERT INTO order_items (order_id, product_id, product_name, quantity, price) VALUES
('ORDER-2026-001', 1, 'Nasi Kotak Ayam Bakar', 2, 25000),
('ORDER-2026-001', 5, 'Rendang Sapi Frozen (500g)', 1, 60000),
('ORDER-2026-002', 2, 'Nasi Kotak Rendang', 3, 30000),
('ORDER-2026-002', 6, 'Ayam Ungkep Bumbu Lengkuas (1 Ekor)', 1, 65000),
('ORDER-2026-002', 8, 'Empal Gentong Sapi Frozen (300g)', 1, 60000),
('ORDER-2026-003', 4, 'Nasi Campur Spesial MAM', 1, 45000),
('ORDER-2026-003', 13, 'Nasi Kotak Empal Serundeng', 1, 35000);

-- 9. KEAMANAN RLS (ROW LEVEL SECURITY) PUBLIC ANON PERMISSION
ALTER TABLE products ENABLE ROW LEVEL SECURITY;
ALTER TABLE orders ENABLE ROW LEVEL SECURITY;
ALTER TABLE order_items ENABLE ROW LEVEL SECURITY;
ALTER TABLE admin_settings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Public Anon All Products" ON products;
CREATE POLICY "Public Anon All Products" ON products FOR ALL TO anon USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Public Anon All Orders" ON orders;
CREATE POLICY "Public Anon All Orders" ON orders FOR ALL TO anon USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Public Anon All Order Items" ON order_items;
CREATE POLICY "Public Anon All Order Items" ON order_items FOR ALL TO anon USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Public Anon All Admin Settings" ON admin_settings;
CREATE POLICY "Public Anon All Admin Settings" ON admin_settings FOR ALL TO anon USING (true) WITH CHECK (true);`;
  ...
  const handleSeedSampleOrders = async () => {
    ...
    try {
      const res = await seedSampleOrdersToSupabase();
      if (res.success) {
        alert("✅ Berhasil membuat 3 contoh transaksi lengkap beserta rincian order_items!");
        loadOrders();
      } else {
        alert(`❌ Gagal menambahkan transaksi contoh: ${res.message || "Error Supabase"}`);
      }
    } catch (err: any) {
        ...
        if (ord.items && Array.isArray(ord.items) && ord.items.length > 0) {
          ord.items.forEach((item: any) => {
            ...
            if (pId && itemSalesMap[pId]) {
              ...
              itemSalesMap[pId] = {
                id: pId,
                name: item.product_name || item.productName || item.name || `Produk #${pId}`,
                price: itemPrice,
                quantitySold: qty,
      ...
      if (matchesSearch && matchesStatus) {
        ...
        if (ord.items && Array.isArray(ord.items)) {
          ord.items.forEach((item: any) => {
            ...
            const pName = item.product_name || item.productName || item.name || (menuItem ? menuItem.name : `Produk #${pId || "Menu"}`);
            const qty = Number(item.quantity) || 1;
                    ...
                    <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-3 py-2 rounded-xl bg-terracotta hover:bg-terracotta-dark text-white font-bold text-xs flex items-center gap-1.5 transition-all shadow-xs shrink-0 cursor-pointer disabled:opacity-50"
                          title="Isi database Supabase dengan 3 transaksi contoh & rincian pesanan"
                        >
                          <Plus className="w-3.5 h-3.5" />
                          <span className="hidden sm:inline">{isSeedingOrders ? "Membuat..." : "+ Transaksi Contoh"}</span>
                        </button>
                                          ...
                                          {ord.items && Array.isArray(ord.items) && ord.items.length > 0 ? (
                                            ...
                                            <div className="py-1.5 flex items-center justify-between text-xs text-espresso-dark">
                                              <div className="flex items-center gap-2">
                                                ...
                                                <span className="font-bold text-espresso-dark/80 italic">
                                                  Paket / Pesanan Catering MAM
                                                </span>
                                              </div>
                                              <div className="text-right font-mono text-xs font-bold text-espresso-dark">
                                                {formatIDR(Number(ord.total_amount))}
                                              </div>
                                            </div>
                                          )}
                              ...
                              {ord.items && Array.isArray(ord.items) && ord.items.length > 0 ? (
                                ...
                                <div className="py-1 flex items-center justify-between text-xs text-espresso-dark">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-semibold text-espresso-dark/80 italic">Paket / Pesanan Catering MAM</span>
                                  </div>
                                  <div className="text-right font-mono text-xs font-bold text-espresso-dark">
                                    {formatIDR(Number(ord.total_amount))}
                                  </div>
